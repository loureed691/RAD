...
class Position:
    ...
    def should_early_exit(self, current_price: float, current_pnl: float) -> tuple[bool, str]:
        ...
        # Made more conservative: 30min (was 15min), -2.5% (was -1.5%), 4 updates (was 3)
        if time_in_trade >= 0.5:  # After 30 minutes
            if current_pnl < -0.025:  # -2.5% loss
                # Check if loss is accelerating (consecutive negative updates)
                if self.consecutive_negative_updates >= 4:
                    return True, 'early_exit_rapid_loss'
        
        # 2. Extended time underwater - position not recovering
        # Made more conservative: 4 hours (was 2 hours), -1.5% (was -1%)
        if time_in_trade >= 4.0:  # After 4 hours
            if current_pnl < -0.015:  # Still -1.5% or worse
                return True, 'early_exit_extended_loss'
        
        # 3. Maximum adverse excursion threshold - cut losses before stop loss
        # Made more conservative: -3.5% drawdown (was -2.5%), -2.5% current (was -2%)
        if self.max_adverse_excursion < -0.035:  # -3.5% drawdown
            if current_pnl < -0.025:  # Current loss > -2.5%
        ...
        # Made more conservative: +1% favorable (was +0.5%), -2% loss (was -1.5%)
        if self.max_favorable_excursion > 0.01:  # Was up at least 1%
            if current_pnl < -0.02:  # Now down -2% or worse
